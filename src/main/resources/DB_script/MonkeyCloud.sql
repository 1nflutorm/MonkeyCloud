...
create or replace trigger user_login_trigger
	...
	for each row 
	execute procedure user_login_exists();

create or replace function user_login_exists()
	returns trigger
	language plpgsql
	as $user_login_exists$
		declare 
			users_count int;
		begin
			select count(1)
			into users_count
			from users 
			where users.user_login = new.user_login;
		if users_count <> 0 then
			raise exception 'Пользователь с логином: % уже существует', new.user_login;
		end if;
	return new;
	end;
	$user_login_exists$;		
